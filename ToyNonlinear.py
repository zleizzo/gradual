...
class ToyNonlinear:
    ...
    def m(self, th, mu):
        decay_rate = self.delta0 ** (mu ** 2)
        # decay_rate = self.delta0 ** (torch.abs(mu))
        return decay_rate * self.mu_star(th) + (1 - decay_rate) * mu
    
    
    def lt_loss(self, th):
        return L(th, self.mu_star(th))
    ...
    def SPGD(self, T, lr, k, H, pert):
        ...
        # Shorter init has slightly better performance but more instable
        warmup = self.d
        # warmup = 2 * self.d + 1
        with torch.no_grad():
            ...
            for t in range(T - warmup):
                ...
                try:
                    ...
                    if torch.linalg.norm(long_term_grad_approx) > 10:
                        long_term_grad_approx /= (torch.linalg.norm(long_term_grad_approx) / 10)
                except RuntimeError:
...
# Run experiments
##############################################################################
def experiment(num_trials, T, lrs, dfo_perts, Hs, spgd_Hs, waits, ks, spgd_perts, d, R, delta0, mean_noise, A = None, b = None, seed = 0):    
    ...
    for r in tqdm(range(num_trials)):
        expt = ToyNonlinear(d, R, delta0, mean_noise, A, b, seed + r)
        ...
        for i in range(len(lrs)):
            for j in range(len(waits)):
                ...
                traj = expt.RGD(T, lr)
                rgd_l[i, j, r]  = np.array([expt.lt_loss(th) for th in traj], dtype=float)
                rgd_th[i, j, r] = np.array(traj)
                rgd_d[i, j, r]  = np.array([torch.linalg.norm(th - expt.opt_th) for th in traj], dtype=float)
...
def make_plots(delta0, mean_noise, T, d, R, num_trials, lrs, waits, ks, dfo_perts, spgd_perts, Hs, spgd_Hs):    
    results = experiment(num_trials, T, lrs, dfo_perts, Hs, spgd_Hs, waits, ks, spgd_perts, d, R, delta0, mean_noise)
    ...
    print(f'SPGD: lr={round(lrs[spgd_best[0]], 3)}, pert={round(spgd_perts[spgd_best[1]], 3)}, H={spgd_Hs[spgd_best[2]]}, k={ks[spgd_best[3]]}')
    
    ###############################################################################
    # Make plots
    ###############################################################################
    expt   = ToyNonlinear(d, R, delta0, mean_noise)
    opt_l  = expt.opt_l
    stab_l = expt.stab_l
    ...
    plt.plot(rgd_l_m, label = f'RGD', color=colors[5])
    plt.fill_between(range(T), rgd_l_m + rgd_l_sem, rgd_l_m - rgd_l_sem, color=colors[5], alpha=0.3)
    
    # plt.plot(dfo_l_m, label = f'DFO: lr={round(lrs[dfo_best[0]], 3)}, pert={round(dfo_perts[dfo_best[1]], 3)}, wait={waits[dfo_best[2]]}', color=colors[3])
    plt.plot(dfo_l_m, label = f'DFO', color=colors[7])
    ...
    plt.plot(pgd_l_m, label = f'PGD', color=colors[4])
    ...
    plt.plot(spgd_l_m, label = f'SPGD', color=colors[2])
    ...
    plt.savefig(f'plots/toy_nonlinear/final/{round(delta0, 3)}.pdf')
...
for i in tqdm(range(len(delta0s))):
    ...
    rgds[i], rgd_ls[i], dfos[i], dfo_ls[i], pgds[i], pgd_ls[i], spgds[i], spgd_ls[i] = make_plots(delta0, mean_noise, T, d, R, num_trials, lrs, waits, ks, dfo_perts, spgd_perts, Hs, spgd_Hs)
